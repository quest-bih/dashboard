...
plot_OC_perc <- function(plot_data, color_palette) {
  ...
  plot_output <- plot_ly(plot_data, x = ~year, y = ~OC_github_perc,
                         name = "GitHub only", type = "bar",
                         marker = list(color = color_palette[3],
                                       line = list(color = "rgb(0,0,0)",
                                                   width = 1.5))) |>
    add_trace(y = ~OC_github_other_perc,
              name = "GitHub and other<br>repository or website<br>(excl. Zenodo and OSF)",
              marker = list(color = color_palette[3],
                            pattern = list(shape = "x"),
                            line = list(color = "rgb(0,0,0)",
                                        width = 1.5))) |>
    add_trace(y = ~OC_zenodo_perc,
              name = "Zenodo",
              marker = list(color = color_palette[6],
                            line = list(color = "rgb(0,0,0)",
                                        width = 1.5))) |>
    add_trace(y = ~OC_osf_perc,
              name = "OSF",
              marker = list(color = color_palette[2],
                            line = list(color = "rgb(0,0,0)",
                                        width = 1.5))) |>
...
make_oddpub_plot_data <- function(data_table, gr) {
  ...
  oddpub_plot_data <- data_table |>
    ...
    summarize(
      ...
      OC_github_other_count = sum(open_code_category_manual == "github and other", na.rm = TRUE),
      OC_zenodo_count = sum(open_code_category_manual == "zenodo", na.rm = TRUE),
      OC_osf_count = sum(open_code_category_manual == "osf", na.rm = TRUE),
    ...
    mutate(      # open_data_perc = round((open_data_manual_count)/total * 100, 1),
           ...
           OC_github_other_perc = round(OC_github_other_count/total_code * 100, 1),
           OC_zenodo_perc = round(OC_zenodo_count/total_code * 100, 1),
           OC_osf_perc = round(OC_osf_count/total_code * 100, 1),
